...
class Client():
    ...
    def read(self, id=None, fields=None, page=-1, per_page=-1): 
        ...
        if (ev.will_run_default()): 
            ...
            if (id != None):
                ...
                res = self._client.send_request('mc_issue_get', body=tostring(root))                                 
            else:
                ...
                res = self._client.send_request('mc_project_get_issues', body=tostring(root))
    ...
    def create(self, params): 
        ...
        if (ev.will_run_default()): 
            ...
            res = self._client.send_request('mc_issue_add', body=tostring(root))
    ...
    def update(self, id, params={}):
        ...
        if (ev.will_run_default()): 
            ...
            res = self._client.send_request('mc_issue_update', body=tostring(root))
    ...
    def delete(self, id):
        ...
        if (ev.will_run_default()):         
            ...
            res = self._client.send_request('mc_issue_delete', body=tostring(root))
            ...
            if ((fields == None or key in fields) and hasattr(rec, key)):
                if (value == 'standard'):
                    ...
                    for item in getattr(rec, key):
                        id = item.id if (hasattr(item, 'id')) else None
                        name = item.name if (hasattr(item, 'name')) else None
                        real_name = item.real_name if (hasattr(item, 'real_name')) else None
                        email = item.email if (hasattr(item, 'email')) else None                        
                        array.append({'id': id, 'name': name, 'real_name': real_name, 'email': email})                
                    ...
                    for item in getattr(rec, key):
                        id = item.id if (hasattr(item, 'id')) else None
                        type_id = item.type.id if (hasattr(item, 'type') and hasattr(item.type, 'id')) else None
                        type_name = item.type.name if (hasattr(item, 'type') and hasattr(item.type, 'name')) else None
